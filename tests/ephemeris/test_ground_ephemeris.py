...
class TestGroundEphemerisBasicProperties:
    ...
    def test_preserves_latitude(self, kitt_peak_obs: GroundEphemeris) -> None:
        """Test that latitude is preserved."""
        assert float(kitt_peak_obs.latitude_deg[0]) == 31.9583

    def test_preserves_longitude(self, kitt_peak_obs: GroundEphemeris) -> None:
        """Test that longitude is preserved."""
        assert float(kitt_peak_obs.longitude_deg[0]) == -111.6

    def test_preserves_height(self, kitt_peak_obs: GroundEphemeris) -> None:
...
class TestGroundEphemerisTimestamps:
    ...
    def test_timestamp_not_none(self, kitt_peak_obs: GroundEphemeris) -> None:
        """Test that timestamps are not None."""
        assert kitt_peak_obs.timestamp is not None

    def test_timestamp_count(self, kitt_peak_obs: GroundEphemeris) -> None:
        """Test that timestamp count is correct."""
        assert len(kitt_peak_obs.timestamp) == 2

    def test_timestamp_begin(self, kitt_peak_obs: GroundEphemeris) -> None:
    ...
    def test_timestamp_end(self, kitt_peak_obs: GroundEphemeris) -> None:
...
class TestGroundEphemerisITRS:
    ...
    def test_itrs_not_none(self, equator_obs: GroundEphemeris) -> None:
        """Test that ITRS object is not None."""
        assert equator_obs.itrs_pv is not None

    def test_position_shape(self, equator_obs: GroundEphemeris) -> None:
        """Test ITRS position array shape."""
        pos = equator_obs.itrs_pv.position
        assert pos.shape == (1, 3)

    def test_position_x_at_equator(self, equator_obs: GroundEphemeris) -> None:
        """Test ITRS X position at equator on prime meridian."""
        pos = equator_obs.itrs_pv.position
        assert np.abs(pos[0, 0] - 6378.137) < 0.01

    def test_position_y_at_equator(self, equator_obs: GroundEphemeris) -> None:
        """Test ITRS Y position at equator on prime meridian."""
        pos = equator_obs.itrs_pv.position
        assert np.abs(pos[0, 1]) < 0.01

    def test_position_z_at_equator(self, equator_obs: GroundEphemeris) -> None:
        """Test ITRS Z position at equator on prime meridian."""
        pos = equator_obs.itrs_pv.position
        assert np.abs(pos[0, 2]) < 0.01

    def test_velocity_shape(self, equator_obs: GroundEphemeris) -> None:
        """Test ITRS velocity array shape."""
        vel = equator_obs.itrs_pv.velocity
        assert vel.shape == (1, 3)

    def test_velocity_x_at_equator(self, equator_obs: GroundEphemeris) -> None:
        """Test ITRS X velocity at equator."""
        vel = equator_obs.itrs_pv.velocity
        assert np.abs(vel[0, 0]) < 0.001

    def test_velocity_y_at_equator(self, equator_obs: GroundEphemeris) -> None:
    ...
    def test_velocity_z_at_equator(self, equator_obs: GroundEphemeris) -> None:
...
class TestGroundEphemerisGCRS:
    ...
    def test_gcrs_not_none(self, mauna_kea_obs: GroundEphemeris) -> None:
        """Test that GCRS object is not None."""
        assert mauna_kea_obs.gcrs_pv is not None

    def test_position_magnitude(self, mauna_kea_obs: GroundEphemeris) -> None:
    ...
    def test_velocity_magnitude(self, mauna_kea_obs: GroundEphemeris) -> None:
...
class TestGroundEphemerisSkyCoords:
    ...
    @pytest.fixture
    def sample_obs(self) -> GroundEphemeris:
        """Create a sample GroundEphemeris for SkyCoord tests."""
        begin = datetime.datetime(2024, 1, 1, 0, 0, 0, tzinfo=datetime.timezone.utc)
        end = datetime.datetime(2024, 1, 1, 0, 0, 0, tzinfo=datetime.timezone.utc)
        return GroundEphemeris(40.0, -74.0, 100.0, begin, end, 3600)

    def test_itrs_skycoord_not_none(self, sample_obs: GroundEphemeris) -> None:
        """Test that ITRS SkyCoord is created."""
        assert sample_obs.itrs is not None

    def test_gcrs_skycoord_not_none(self, sample_obs: GroundEphemeris) -> None:
        """Test that GCRS SkyCoord is created."""
        assert sample_obs.gcrs is not None

    def test_earth_skycoord_not_none(self, sample_obs: GroundEphemeris) -> None:
        """Test that Earth SkyCoord is created."""
        assert sample_obs.earth is not None

    def test_sun_skycoord_not_none(self, sample_obs: GroundEphemeris) -> None:
        """Test that Sun SkyCoord is created."""
        assert sample_obs.sun is not None

    def test_moon_skycoord_not_none(self, sample_obs: GroundEphemeris) -> None:
...
class TestGroundEphemerisSunMoon:
    ...
    def test_sun_not_none(self, equator_obs: GroundEphemeris) -> None:
        """Test that Sun object is not None."""
        assert equator_obs.sun_pv is not None

    def test_sun_distance(self, equator_obs: GroundEphemeris) -> None:
    ...
    def test_moon_not_none(self, equator_obs: GroundEphemeris) -> None:
        """Test that Moon object is not None."""
        assert equator_obs.moon_pv is not None

    def test_moon_distance(self, equator_obs: GroundEphemeris) -> None:
...
class TestGroundEphemerisValidation:
    ...
    @pytest.fixture
    def valid_times(self) -> tuple[datetime.datetime, datetime.datetime]:
    ...
    def test_invalid_latitude_high(
        self, valid_times: tuple[datetime.datetime, datetime.datetime]
    ) -> None:

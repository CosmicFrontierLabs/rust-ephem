...
class TestBasicConstruction:
    """Tests for FileEphemeris initialization and basic properties."""

    def test_basic_initialization(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert eph is not None

    def test_timestamp_count(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert eph.timestamp is not None
        assert len(eph.timestamp) == 31  # 30 min / 60 s + 1

    def test_step_size_property(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert eph.step_size == STEP


class TestSourceMetadata:
    """Tests for source metadata properties."""

    def test_source_frame_from_header(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert eph.source_frame == "J2000"

    def test_file_path_property(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert eph.file_path == seconds_file

    def test_source_units_default_to_km(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert eph.source_position_unit == "km"
        assert eph.source_velocity_unit == "km/s"

    def test_polar_motion_defaults_false(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert eph.polar_motion is False

    def test_polar_motion_flag_propagates(self, seconds_file: str) -> None:
        eph = FileEphemeris(
...
class TestGcrsOutput:
    """Tests for GCRS position/velocity output."""

    def test_gcrs_pv_shape(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        gcrs_pv: PositionVelocityData = eph.gcrs_pv
        assert gcrs_pv.position.shape == (31, 3)
        assert gcrs_pv.velocity.shape == (31, 3)

    def test_gcrs_position_in_leo_range(self, seconds_file: str) -> None:
    ...
    def test_gcrs_orbital_speed_plausible(self, seconds_file: str) -> None:
...
class TestItrsOutput:
    """Tests for ITRS position/velocity output."""

    def test_itrs_pv_shape(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        itrs_pv: PositionVelocityData = eph.itrs_pv
        assert itrs_pv.position.shape == (31, 3)
        assert itrs_pv.velocity.shape == (31, 3)

    def test_gcrs_and_itrs_positions_differ(self, seconds_file: str) -> None:
...
class TestRawFileData:
    """Tests for raw file data access (file_pv, file_timestamp)."""

    def test_file_pv_row_count(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert eph.file_pv.position.shape == (7, 3)
        assert eph.file_pv.velocity.shape == (7, 3)

    def test_file_pv_first_row_values(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
    ...
    def test_file_timestamp_count(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert len(eph.file_timestamp) == 7

    def test_file_timestamp_utc_aware(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        for ts in eph.file_timestamp:
            assert ts.tzinfo is not None

    def test_file_timestamp_first_and_last(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
    ...
    def test_file_timestamp_count_differs_from_output(self, seconds_file: str) -> None:
...
class TestTimestampFormats:
    """Tests for timestamp format variants."""

    def test_natural_language_epoch_parses(self, natural_lang_epoch_file: str) -> None:
        eph = FileEphemeris(
            natural_lang_epoch_file, begin=BEGIN, end=END, step_size=STEP
        )
        assert eph.file_pv.position.shape[0] == 7
        assert eph.file_pv.position[0, 0] == pytest.approx(7000.0, rel=1e-5)

    def test_iso8601_explicit_format(self, iso8601_file: str) -> None:
        eph = FileEphemeris(
    ...
    def test_iso8601_auto_detected(self, iso8601_file: str) -> None:
    ...
    def test_days_time_format(self, days_file: str) -> None:
        eph = FileEphemeris(
    ...
    def test_seconds_time_format_explicit(self, seconds_file: str) -> None:
        eph = FileEphemeris(
            seconds_file, begin=BEGIN, end=END, step_size=STEP, time_format="seconds"
        )
        assert eph.file_pv.position.shape[0] == 7

    def test_explicit_epoch_override(self, no_epoch_file: str) -> None:
...
class TestUnitConversion:
    ...
    def test_meters_source_unit_preserved(self, meters_file: str) -> None:
        eph = FileEphemeris(
            meters_file,
            begin=BEGIN,
            end=END,
    ...
    def test_meters_file_pv_converted_to_km(self, meters_file: str) -> None:
    ...
    def test_meters_gcrs_pv_shape(self, meters_file: str) -> None:
        eph = FileEphemeris(
            meters_file,
            begin=BEGIN,
            end=END,
...
class TestEarthFixedFrame:
    """Tests for ECEF frame handling."""

    def test_ecef_frame_from_header(self, ecef_file: str) -> None:
    ...
    def test_frame_override_parameter(self, seconds_file: str) -> None:
...
class TestCommentLineTolerance:
    """Tests for comment line handling."""

    def test_comment_lines_skipped(self, tmp_path: Any) -> None:
...
class TestErrors:
    ...
    def test_time_range_before_file_data_raises(self, seconds_file: str) -> None:
    ...
    def test_seconds_format_without_epoch_raises(self, no_epoch_file: str) -> None:
    ...
    def test_unknown_position_unit_raises(self, seconds_file: str) -> None:
    ...
    def test_unknown_velocity_unit_raises(self, seconds_file: str) -> None:
...
class TestEphemerisAbcMembership:
    """Tests for Ephemeris ABC membership."""

    def test_isinstance_ephemeris(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert isinstance(eph, Ephemeris)

    def test_issubclass_ephemeris(self) -> None:
        assert issubclass(FileEphemeris, Ephemeris)


class TestSunMoon:
    """Tests for Sun/Moon properties."""

    def test_sun_and_moon_accessible(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        assert eph.sun is not None
        assert eph.moon is not None

    def test_sun_pv_shape(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
...
class TestGeodeticProperties:
    """Tests for geodetic properties (latitude, height)."""

    def test_latitude_deg_shape(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        lat = eph.latitude_deg
        assert lat is not None
        assert len(lat) == 31

    def test_latitude_deg_in_range(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        lat = eph.latitude_deg
        assert lat is not None
    ...
    def test_height_km_plausible(self, seconds_file: str) -> None:
...
class TestIndexMethod:
    ...
    def test_index_exact_time(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)
        target = datetime(2024, 1, 1, 0, 15, 0, tzinfo=timezone.utc)
        assert eph.index(target) == 15  # 15 min = 15 × 60 s steps

    def test_index_between_steps(self, seconds_file: str) -> None:
        eph = FileEphemeris(seconds_file, begin=BEGIN, end=END, step_size=STEP)

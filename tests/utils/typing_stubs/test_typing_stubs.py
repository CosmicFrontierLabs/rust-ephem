...
class TestModuleLevelFunctions:
    ...
    def test_get_cache_dir_returns_string_type(self) -> None:
        cache_dir: str = rust_ephem.get_cache_dir()
        assert isinstance(cache_dir, str)

    def test_get_cache_dir_is_non_empty(self) -> None:
        cache_dir: str = rust_ephem.get_cache_dir()
        assert len(cache_dir) > 0

    def test_is_planetary_ephemeris_initialized_returns_bool(self) -> None:
        is_init: bool = rust_ephem.is_planetary_ephemeris_initialized()
        assert isinstance(is_init, bool)

    def test_get_tai_utc_offset_accepts_datetime_returns_optional_float(self) -> None:
        test_time = datetime(2021, 1, 1, 12, 0, 0)
        tai_utc: float | None = rust_ephem.get_tai_utc_offset(test_time)
        assert tai_utc is None or isinstance(tai_utc, float)

    def test_get_tai_utc_offset_positive_if_present(self) -> None:
        test_time = datetime(2021, 1, 1, 12, 0, 0)
        tai_utc: float | None = rust_ephem.get_tai_utc_offset(test_time)
        if tai_utc is None:
    ...
    def test_get_ut1_utc_offset_with_datetime_returns_float(self) -> None:
        test_time = datetime(2021, 1, 1, 12, 0, 0)
        ut1_utc: float = rust_ephem.get_ut1_utc_offset(test_time)
        assert isinstance(ut1_utc, float)

    def test_get_polar_motion_with_datetime_returns_tuple_of_two(self) -> None:
        test_time = datetime(2021, 1, 1, 12, 0, 0)
        x, y = rust_ephem.get_polar_motion(test_time)
        assert hasattr((x, y), "__iter__") and len((x, y)) == 2

    def test_get_polar_motion_components_are_floats(self) -> None:
        test_time = datetime(2021, 1, 1, 12, 0, 0)
        x, y = rust_ephem.get_polar_motion(test_time)
        assert isinstance(x, float)

    def test_get_polar_motion_second_component_is_float(self) -> None:
        test_time = datetime(2021, 1, 1, 12, 0, 0)
        _, y = rust_ephem.get_polar_motion(test_time)
        assert isinstance(y, float)

    def test_oem_ephemeris_has_height_km_attribute(self) -> None:
...
class TestTLEEphemerisTyping:
    ...
    @pytest.fixture
    def tle_ephem(self) -> rust_ephem.TLEEphemeris:
    ...
    def test_constructor_accepts_parameters_and_returns_object(self) -> None:
        begin = datetime(2021, 1, 1)
        end = datetime(2021, 1, 2)
    ...
    def test_teme_pv_has_velocity_attribute(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        teme_pv = tle_ephem.teme_pv
        if teme_pv is None:
    ...
    def test_teme_pv_position_shape_has_three_columns(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        teme_pv = tle_ephem.teme_pv
        if teme_pv is None:
    ...
    def test_teme_pv_velocity_shape_has_three_columns(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        teme_pv = tle_ephem.teme_pv
        if teme_pv is None:
    ...
    def test_latitude_quantity_cached(self, tle_ephem: rust_ephem.TLEEphemeris) -> None:
    ...
    def test_height_quantity_cached(self, tle_ephem: rust_ephem.TLEEphemeris) -> None:
        h1 = tle_ephem.height
        h2 = tle_ephem.height
    ...
    def test_gcrs_pv_property_not_none(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        gcrs_pv = tle_ephem.gcrs_pv
        assert gcrs_pv is not None

    def test_gcrs_pv_has_position_and_velocity_shape(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        gcrs_pv = tle_ephem.gcrs_pv
        if gcrs_pv is None:
    ...
    def test_gcrs_pv_position_three_columns(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        gcrs_pv = tle_ephem.gcrs_pv
        if gcrs_pv is None:
    ...
    def test_itrs_pv_property_not_none(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        itrs_pv = tle_ephem.itrs_pv
        assert itrs_pv is not None

    def test_itrs_pv_has_position_attribute(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        itrs_pv = tle_ephem.itrs_pv
        if itrs_pv is None:
    ...
    def test_itrs_pv_has_velocity_attribute(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        itrs_pv = tle_ephem.itrs_pv
        if itrs_pv is None:
    ...
    def test_itrs_property_not_none(self, tle_ephem: rust_ephem.TLEEphemeris) -> None:
        assert tle_ephem.itrs is not None

    def test_gcrs_property_not_none(self, tle_ephem: rust_ephem.TLEEphemeris) -> None:
        assert tle_ephem.gcrs is not None

    def test_earth_property_not_none(self, tle_ephem: rust_ephem.TLEEphemeris) -> None:
        assert tle_ephem.earth is not None

    def test_sun_property_not_none(self, tle_ephem: rust_ephem.TLEEphemeris) -> None:
        assert tle_ephem.sun is not None

    def test_moon_property_not_none(self, tle_ephem: rust_ephem.TLEEphemeris) -> None:
        assert tle_ephem.moon is not None

    def test_timestamp_property_is_ndarray_when_present(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        import numpy as np
    ...
    def test_timestamp_property_is_non_empty_when_present(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        timestamps = tle_ephem.timestamp
        if timestamps is None:
    ...
    def test_height_km_and_height_m_consistent(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
    ...
    def test_timestamp_first_element_has_year_attribute_when_present(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        timestamps = tle_ephem.timestamp
    ...
    def test_get_body_accepts_string_and_returns_object(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        sun_coord = tle_ephem.get_body("sun")
        assert sun_coord is not None

    def test_get_body_pv_accepts_string_and_returns_with_position(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        sun_pv = tle_ephem.get_body_pv("sun")
        assert sun_pv is not None

    def test_get_body_pv_has_position_attribute(
        self, tle_ephem: rust_ephem.TLEEphemeris
    ) -> None:
        sun_pv = tle_ephem.get_body_pv("sun")
        if sun_pv is None:
...
class TestSPICEEphemerisTyping:
    ...
    def test_spice_ephemeris_class_exists(self) -> None:
        assert hasattr(rust_ephem, "SPICEEphemeris")

    def test_spice_ephemeris_is_callable(self) -> None:
        assert callable(rust_ephem.SPICEEphemeris)

    def test_spice_ephemeris_has_height_km_attribute(self) -> None:
...
class TestGroundEphemerisTyping:
    ...
    @pytest.fixture
    def ground_ephem(self) -> rust_ephem.GroundEphemeris:
    ...
    def test_gcrs_pv_not_none(self, ground_ephem: rust_ephem.GroundEphemeris) -> None:
        gcrs_pv = ground_ephem.gcrs_pv
        assert gcrs_pv is not None

    def test_gcrs_pv_has_position_attribute(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        gcrs_pv = ground_ephem.gcrs_pv
        if gcrs_pv is None:
            pytest.skip("gcrs_pv is None")
    ...
    def test_itrs_pv_not_none(self, ground_ephem: rust_ephem.GroundEphemeris) -> None:
    ...
    def test_itrs_property_not_none(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        assert ground_ephem.itrs is not None

    def test_gcrs_property_not_none(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        assert ground_ephem.gcrs is not None

    def test_sun_property_not_none(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        assert ground_ephem.sun is not None

    def test_moon_property_not_none(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        assert ground_ephem.moon is not None

    def test_sun_pv_property_not_none(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        sun_pv = ground_ephem.sun_pv
        assert sun_pv is not None

    def test_sun_pv_has_position_attribute(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        sun_pv = ground_ephem.sun_pv
        if sun_pv is None:
    ...
    def test_moon_pv_property_not_none(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        moon_pv = ground_ephem.moon_pv
        assert moon_pv is not None

    def test_moon_pv_has_position_attribute(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        moon_pv = ground_ephem.moon_pv
        if moon_pv is None:
    ...
    def test_latitude_deg_has_shape(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        lat = ground_ephem.latitude_deg
        assert hasattr(lat, "shape")

    def test_longitude_deg_has_shape(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        lon = ground_ephem.longitude_deg
        assert hasattr(lon, "shape")

    def test_height_m_has_shape(self, ground_ephem: rust_ephem.GroundEphemeris) -> None:
        height = ground_ephem.height_m
        assert hasattr(height, "shape")

    def test_lat_lon_height_shapes_are_tuples(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        lat = ground_ephem.latitude_deg
        assert isinstance(lat.shape, tuple)

    def test_longitude_shape_is_tuple(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        lon = ground_ephem.longitude_deg
        assert isinstance(lon.shape, tuple)

    def test_height_shape_is_tuple(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        height = ground_ephem.height_m
        assert isinstance(height.shape, tuple)

    def test_latitude_first_value_matches(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        lat = ground_ephem.latitude_deg
        assert float(lat[0]) == pytest.approx(37.4)

    def test_longitude_first_value_matches(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        lon = ground_ephem.longitude_deg
        assert float(lon[0]) == pytest.approx(-122.1)

    def test_height_first_value_matches(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        height = ground_ephem.height_m
        assert float(height[0]) == pytest.approx(0.1)

    def test_latitude_quantity_cached(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
    ...
    def test_height_quantity_cached(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
    ...
    def test_height_km_first_value_matches(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
        height_km = ground_ephem.height_km
        assert float(height_km[0]) == pytest.approx(0.1 / 1000.0)

    def test_obsgeoloc_property_can_be_none_or_has_len(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
    ...
    def test_obsgeovel_property_can_be_none_or_has_len(
        self, ground_ephem: rust_ephem.GroundEphemeris
    ) -> None:
...
class TestConstraintTyping:
    ...
    def test_from_json_static_method_exists_and_parses(self) -> None:
        json_str = '{"type": "sun", "min_angle": 10.0}'
        constraint: rust_ephem.Constraint = rust_ephem.Constraint.from_json(json_str)
...
class TestPositionVelocityDataTyping:
    ...
    def test_position_velocity_data_has_position_attr(self) -> None:
        begin = datetime(2021, 1, 1)
        end = datetime(2021, 1, 2)
        tle = rust_ephem.TLEEphemeris(
            TLE1, TLE2, begin, end, step_size=3600, polar_motion=False
        )
        pv: rust_ephem.PositionVelocityData = tle.teme_pv
        if pv is None:
            pytest.skip("pv is None")
        assert hasattr(pv, "position")

    def test_position_velocity_data_has_velocity_attr(self) -> None:
        begin = datetime(2021, 1, 1)
        end = datetime(2021, 1, 2)
        tle = rust_ephem.TLEEphemeris(
            TLE1, TLE2, begin, end, step_size=3600, polar_motion=False
    ...
    def test_position_velocity_data_has_position_unit_attr(self) -> None:
        begin = datetime(2021, 1, 1)
        end = datetime(2021, 1, 2)
        tle = rust_ephem.TLEEphemeris(
            TLE1, TLE2, begin, end, step_size=3600, polar_motion=False
    ...
    def test_position_velocity_data_has_velocity_unit_attr(self) -> None:
        begin = datetime(2021, 1, 1)
        end = datetime(2021, 1, 2)
        tle = rust_ephem.TLEEphemeris(
            TLE1, TLE2, begin, end, step_size=3600, polar_motion=False
    ...
    def test_position_velocity_data_position_unit_matches(self) -> None:
        begin = datetime(2021, 1, 1)
        end = datetime(2021, 1, 2)
        tle = rust_ephem.TLEEphemeris(
            TLE1, TLE2, begin, end, step_size=3600, polar_motion=False
    ...
    def test_position_velocity_data_velocity_unit_matches(self) -> None:
        begin = datetime(2021, 1, 1)
        end = datetime(2021, 1, 2)
        tle = rust_ephem.TLEEphemeris(
            TLE1, TLE2, begin, end, step_size=3600, polar_motion=False

...
* ``osculating_elements_from_state(position_km, velocity_km_s, mu_km3_s2=WGS72_EARTH_MU_KM3_S2)`` — Derive instantaneous classical osculating elements from one inertial Cartesian state.
  ...
  * ``eccentricity``
  * ``inclination_deg``
  * ``right_ascension_of_ascending_node_deg``
  ...
  * ``gravitational_parameter_km3_s2``
  ...
  for example, the returned orientation angles are relative to the GCRS axes.

  These are instantaneous two-body osculating elements. They are not
  interchangeable with the mean elements in a TLE, and a later state should not
  be generated by changing only the anomaly under a perturbed propagator.

  For singular classical-element cases, undefined angles use stable
  conventions: circular inclined states report argument of periapsis as zero
  and true anomaly as argument of latitude; eccentric equatorial states report
  right ascension of the ascending node as zero and argument of periapsis as
  longitude of periapsis; circular equatorial states report both undefined
  orientation angles as zero and true anomaly as true longitude.

...
impl TLEEphemeris {
    ...
    fn sun_angular_radius_quantity(&self, py: Python) -> PyResult<Py<PyAny>> {
        let angular_radii_array = self.sun_angular_radius(py)?;
    ...
    fn moon_angular_radius_quantity(&self, py: Python) -> PyResult<Py<PyAny>> {
        let angular_radii_array = self.moon_angular_radius(py)?;
        let astropy = py.import("astropy.units")?;
        let quantity_class = astropy.getattr("Quantity")?;
        let deg_unit = astropy.getattr("deg")?;
    ...
    fn earth_angular_radius_quantity(&self, py: Python) -> PyResult<Py<PyAny>> {
        let angular_radii_array = self.earth_angular_radius(py)?;
        let astropy = py.import("astropy.units")?;
        let quantity_class = astropy.getattr("Quantity")?;
        let deg_unit = astropy.getattr("deg")?;

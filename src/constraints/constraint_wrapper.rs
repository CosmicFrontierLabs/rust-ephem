...
fn rotate_radec_with_euler_zyx(
    ra_deg: f64,
    dec_deg: f64,
    roll_deg: f64,
    pitch_deg: f64,
    ...
    let x = (cy * cp) * v[0] + (cy * sp * sr - sy * cr) * v[1] + (cy * sp * cr + sy * sr) * v[2];
    let y = (sy * cp) * v[0] + (sy * sp * sr + cy * cr) * v[1] + (sy * sp * cr - cy * sr) * v[2];
    let z = (-sp) * v[0] + (cp * sr) * v[1] + (cp * cr) * v[2];
...
impl ConstraintEvaluator for BoresightOffsetEvaluator {
    fn evaluate(
        ...
        let (offset_ra, offset_dec) = rotate_radec_with_euler_zyx(
            target_ra,
            target_dec,
            self.roll_deg,
            self.pitch_deg,
    ...
    fn in_constraint_batch(
        ...
        for (&ra, &dec) in target_ras.iter().zip(target_decs.iter()) {
            let (offset_ra, offset_dec) =
                rotate_radec_with_euler_zyx(ra, dec, self.roll_deg, self.pitch_deg, self.yaw_deg);

...
pub fn calculate_sun_altitudes_batch(
    ...
    // Convert observer positions from GCRS to ITRS for geodetic coordinates
    let itrs_data = convert_frames(
        &obs_filtered,
        &times_filtered,
        Frame::GCRS,
    ...
    // Convert ITRS positions to geodetic coordinates
    let positions_slice = itrs_data.slice(s![.., 0..3]);
    let positions_array = positions_slice.to_owned();
    let (lats_deg, lons_deg, heights_km) = ecef_to_geodetic_deg(&positions_array);

    // Calculate relative Sun positions (Sun as seen from observer)
    let sun_relative = &sun_filtered - &obs_filtered;

    // Convert Sun positions to RA/Dec
    let sun_positions_xyz = sun_relative.slice(s![.., 0..3]).to_owned();
    let sun_radec = crate::utils::vector_math::cartesian_to_radec_batch(&sun_positions_xyz);

    // Calculate altitude for each time
    for i in 0..n_times {
        let sun_ra_rad = sun_radec[[i, 0]].to_radians();
        let sun_dec_rad = sun_radec[[i, 1]].to_radians();
        ...
        // Use SOFA apparent-place routine for topocentric alt/az
